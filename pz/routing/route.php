...
class Route
{
    ...
    protected bool $was_checked = false;

    /** Determines if the route requires authentication based on its privacy settings and HTTP methods. */
    protected bool $requires_authentication {
        get => $this->privacy->requiresAuth()
            && ($this->hasMethod(Method::POST) || $this->hasMethod(Method::PUT) || $this->hasMethod(Method::DELETE));
    }
    ...
    public function __construct(
        ...
        if ($controller !== null) {
            if ($function === null) {
                ...
                . $path
                . ' - Controller defined but no method to reach was given.');
            }
            if (!is_subclass_of($this->controller, 'pz\Controller')) {
                throw new InvalidArgumentException('Invalid action : '
                . $path
                . ' - The route\'s controller must be a subclass of Controller.');
            }
        ...
        $this->response = new Response(false, ResponseCode::InternalServerError);
    }
    ...
    public function check(Request $request): Response
    {
        ...
        if ($this->privacy->requiresLogin() && !$request->isLoggedIn()) {
     ...
     * Simple wrapper around serve_request() that checks if the route was checked before serving the request.
     * Allows child classes to override the serve() method and add additional checks before serving the request.
     *
    ...
    public function serve(Request $request): Response
    {
        if (!$this->was_checked) {
            ...
            . $this->path
            . ' - The route must be checked before serving the request. Call the check() method first.');
        }
    ...
    /**
     * Serves the request by invoking the appropriate controller and function, handling redirects, and returning a response.
     * This is the method doing the actual work of serving the request, and is called by the serve() method after the route has been checked.
     *
     * @return Response The HTTP response generated by the controller.
     */
    protected function serve_request(): Response
    {
        $this->find_redirects();
        ...
        if ($controller == null || $function == null) {
            // Nothing to do, just return the default response (which is an error)
            return $this->response;
        }
        ...
        if (!$controller_response instanceof Response) {
            ...
            . $controller_response
            . ' - The controller\'s function must return an instance of Response.');
        }
     ...
     *
     * @return string|null The function associated with the route, or null if not set.
     */
    ...
    protected function respondWithBadMethod($method)
    {
        $this->response = new Response(
            ...
            . $method->value
            . ' : This resource only accepts '
            . implode(',', array_map(fn($m) => $m->value, $this->methods))
            . '.',
