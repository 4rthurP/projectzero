...
class Nonce
{
    ...
    public function __construct($user_id)
    {
        ...
        if ($result->num_rows == 0) {
            $this->makeNonce();
            $id = Database::execute(
                'INSERT INTO nonces (user_id, nonce, expiration) VALUES (?, ?, ?)',
                'iss',
                $this->user_id,
                $this->nonce,
                $this->nonce_expiration->format('Y-m-d H:i:s'),
            );
            $this->nonce_id = $id;
            $this->is_valid = true;
    ...
    public function checkNonce($nonce)
    {
        ...
        if (!$this->was_expired && hash_equals($old_nonce, $nonce)) {
            $this->is_valid = true;
        } else {
            $this->is_valid = false;
        }
    ...
    private function useNonce()
    {
        $this->makeNonce();
        Database::execute(
            'UPDATE nonces SET nonce = ?, expiration = ? WHERE id = ?',
            'ssi',
            $this->nonce,
            $this->nonce_expiration->format('Y-m-d H:i:s'),
            $this->nonce_id,
