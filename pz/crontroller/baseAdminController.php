...
class BaseAdminController extends Controller {
    public function initialize_database(Request $request): Response {
        ...
        $result = $this->generateApplicationDatabase(explode(',', $model_list), true, true);
        return new Response(true, ResponseCode::Ok, 'The database has been initialized.', null, ['result' => $result]);
    }
    ...
    public function update_database(Request $request): Response {
        ...
        $result = $this->generateApplicationDatabase(explode(',', $model_list), false, false);
        return new Response(true, ResponseCode::Ok, 'The database has been updated.', null, ['result' => $result]);
    }
    ...
    public function check_database(Request $request): Response {
        ...
        return new Response(true, ResponseCode::Ok, 'Database check completed.', null, ['result' => $result]);
    }
    ...
    public function checkApplicationDBAdequation(Array $model_list, bool $generate_if_not_successful = false): Array {
        $result = [];
        $success = true;
        foreach($model_list as $model => $model_controller) {
